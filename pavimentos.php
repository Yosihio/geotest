...
$totalPages_agregados = ceil($totalRows_agregados/$maxRows_agregados)-1;$maxRows_agregados = 10;
$pageNum_agregados = 0;
if (isset($_GET['pageNum_agregados'])) {
  $pageNum_agregados = $_GET['pageNum_agregados'];
}
...
mysql_select_db($database_conex, $conex);
$query_agregados = "SELECT * FROM tblproductos WHERE tblproductos.intCategoria =7";
$query_limit_agregados = sprintf("%s LIMIT %d, %d", $query_agregados, $startRow_agregados, $maxRows_agregados);
$agregados = mysql_query($query_limit_agregados, $conex) or die(mysql_error());
$row_agregados = mysql_fetch_assoc($agregados);
  ...
  <div id="container">
	...
	$imgSizer->resize();
        ...
        <div>
            ...
            <div class="price"><?php echo $row_agregados['strClave']; ?></div>
            
            
            <div class="cart"><a href="producto.php?producto=<?php echo $row_pavimentos['idProducto']; ?>">
              <input name="Enviar" type="submit" class="button"  value="Ver producto" />
            </div>
          </div>
